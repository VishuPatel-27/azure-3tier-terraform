...
/*
 * Goal struct represents a goal item with ID and Name fields
 * Basically a model for our goals table in the database
 * It stores the ID and Name of each goal
 */
type Goal struct {
	ID   int    `json:"ID"`
...
/*
 * Prometheus metrics
 * Counters for tracking API requests
 * and specific actions like adding/removing goals
 * Also a counter vector for tracking HTTP requests by path
 * This helps monitor usage and performance of the API
 */
var (
